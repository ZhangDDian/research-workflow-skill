...
7. **证据-推断分离** — 发现列表里，实证数据（有样本量/实验）、咨询预测（框架/问卷/专家意见）、案例（单一公司）是否分开呈现？三者混在同一列表平级罗列 = 不合格
8. **适用边界** — 结论是否标注适用范围（阶段/行业/规模）？缺适用边界的结论无法执行，必须补
9. **反面证据** — 是否至少包含 2 条反面证据或失败模式？纯正面报告 = 不完整
10. **建议锚定** — 每条推荐动作是否关联到具体证据编号？无锚点的建议标注为推测
